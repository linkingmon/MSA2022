function [f_hat, sigma_hat, sigma_w_hat] = HTP(x_tilde)
    % x_tilde [1 x N]
    f_hat = [];        % 1 x P
    sigma_hat = [];    % 1 x P
    sigma_w_hat = [];  % 1 x 1
    ...
    sup_f = [];
    for i_P = 1 : 5
        [~, sup_f] = maxk(f_sparse + A'*(x_tilde.'-A*f_sparse)/G,s); % sup_f: s x 1
        f_sparse = zeros(G,1);
        f_sparse(sup_f) = real(pinv(A(:,sup_f))*x_tilde.');
        f_hat = f_grid(sup_f);
        sigma_hat = real(f_sparse(sup_f));
        if plot_on
            subplot(6,1,i_P+1)
